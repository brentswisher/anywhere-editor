...
export default function ImageInput( props ) {
	const fileRef = React.createRef(),
		resizeImage = function( image, width, quality ) {
			const canvas = document.createElement( 'canvas' ),
				...
				stepToWidth = function( input, width ) {
					const steps = Math.ceil( Math.log( input.width / width ) / Math.log( 2 ) ),
		...
		saveFile = function( e ) {
			const file = e.target.files[ 0 ],
				...
				quality = Math.min( 85, Math.max( 20, parseInt( ( maxImageSize / file.size ) * 100 ) ) );
			let fileSaved = true;
			props.setError( '' );

			// Only process image files.
			if ( ! file.type.match( 'image.*' ) ) {
				props.setError( 'The file you tried to upload is an invalid file type, please upload a different image' );
				fileSaved = false;
			}
				...
				reader.onload = ( loadEvent ) => {
					...
					img.onload = ( evt ) => {
						...
						if ( evt.target.width < minWidth ) {
							props.setError( `This image is not wide enough, please upload an image that is at least ${ minWidth } px wide.` );
							fileRef.current.value = '';
						} else {
							const newSrc = {};
							props.sizes.map( ( size ) => {
								if ( evt.target.naturalWidth === size && file.size <= maxImageSize * 1.1 ) {
	...
	return (
		<fieldset>
			...
			<input type="file" id="photo" onChange={ saveFile } autoFocus={ true } ref={ fileRef } />
			<TextInput fieldName="photoAlt" fieldLabel="Photo Alt Text" value={ props.alt } onChange={ ( e ) => props.onChange( 'alt', e ) } />
			{ typeof props.caption === 'string' &&
				<ContentEditor fieldName="photoCaption" fieldLabel="Photo Caption" value={ props.caption } onChange={ ( e ) => props.onChange( 'caption', e ) } />
			}
		</fieldset>
	);
}